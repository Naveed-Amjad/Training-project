    ...
    <Navbar style={{ position: 'fixed', zIndex: '10' }} className="bg-body-tertiary w-100">
      <Container>
        ...
        <Navbar.Collapse className="justify-content-end">
          ...
          <Nav.Link>
            {
              role && <>{adminNotifications?.length ? <img src={Bell} onClick={() => setShowNotificationsModel(true)} /> : ''}
          ...
          <Navbar.Text style={{ marginLeft: '10px' }}>{role ? `${name}` : <Button style={{ height: '50px', width: '60px', padding: '3px 0px 3px 0px', background: '#fff', color: '#007BFF', border: 'none' }} onClick={() => { nav('/login'); }}>Login</Button>}</Navbar.Text>
